...
print(stock_data['week\FDX'])
#************************show the difference between 2 company*****************************

fig, ax = plt.subplots(figsize=(12,5))
ax.grid(True)
ax1 = stock_data['week\AMZN'].plot(color='green', grid=True, label='KO')
ax2 = ax1.twiny()
ax2.set_xlim(0, 4)
ax2.set_xlabel("njareb")
...
plt.show()
ax.axvline("2015-11-27", color='green', linestyle='-')
#plt.plot(stock_data['week\KO'], color='purple', label='kouka')
#plt.axvline(x=np.pi,color='gray',linestyle='--')
#plt.plot([stock_data['week\FDX'][0],stock_data['week\FDX'][15]], [stock_data['Date'][0],stock_data['Date'][15]])
#plt.plot([stock_data['week\FDX'][180]],[stock_data['week\FDX'][400]])
...
# fit model
history =model.fit(X, y, epochs=2000, verbose=0,validation_split=0.6)
...
def all_perdection(rag):
    ...
    while j < 17:
        ...
        if final_array[j][0][0][1] - final_array[j][0][0][0] < 80:
...
Y1 = [ragroug[8][0][0][0],ragroug[9][0][0][0],ragroug[10][0][0][0],ragroug[11][0][0][0],ragroug[12][0][0][0],ragroug[13][0][0][0],ragroug[14][0][0][0],ragroug[15][0][0][0]]
Y2 = [ragroug[8][0][0][1],ragroug[9][0][0][1],ragroug[10][0][0][1],ragroug[11][0][0][1],ragroug[12][0][0][1],ragroug[13][0][0][1],ragroug[14][0][0][1],ragroug[15][0][0][1]]

Y2018 = [ragroug[15][0][0][0],ragroug[16][0][0][0],ragroug[17][0][0][0],ragroug[18][0][0][0]]
Y2018_2 = [ragroug[15][0][0][1],ragroug[16][0][0][1],ragroug[17][0][0][1],ragroug[18][0][0][1]]
...
slope1, intercept1, r_value, p_value, std_err = stats.linregress(X1, Y2)
def predict1(x):
...
slope2018, intercept2018, r_value, p_value, std_err = stats.linregress(X2018, Y2018)
print(intercept)
def predict2018(x):
    return slope2018 * x + intercept2018

slope12018_2, intercept2018_2, r_value, p_value, std_err = stats.linregress(X2018, Y2018_2)
def predict2018_2(x):
    return slope12018_2 * x + intercept2018_2
...
fig, ax = plt.subplots(figsize=(12,5))
ax.grid(True)
ax1 = stock_data['week\AMZN'].plot(color='green', grid=True, label='AMZON')
#ax.axvline("2015-11-27", color='green', linestyle='-')
ax2 = ax1.twiny()
...
y2 = [predict(6), predict(8)]
y3 = [predict1(6), predict1(8)]

y2018 = [predict2018(8), predict2018(9)]
y2018_2 = [predict2018_2(8), predict2018_2(9)]

plt.plot(x2, y2)
plt.plot(x2, y3)

plt.plot(x2018, y2018)
plt.plot(x2018, y2018_2)
...
"""
plt.annotate("",xy=(7,750), xytext=(7,900),
             arrowprops=dict(arrowstyle="<|-|>"))

plt.annotate("",xy=(7.5,900), xytext=(7.5,1050),
             arrowprops=dict(arrowstyle="<|-|>"))
             """

plt.annotate("",xy=(7,ragroug[13][0][0][0]), xytext=(7,ragroug[13][0][0][1]),
             arrowprops=dict(arrowstyle="<|-|>"))

plt.annotate("",xy=(7.5,ragroug[13][0][0][1]), xytext=(7.5,ragroug[14][0][0][1]),
             arrowprops=dict(arrowstyle="<|-|>"))

print(ragroug[13][0][0][0])
